# 🎯 **Day 12 – Bubble Sort (Questions Only)**

### 🏫 **Class Questions (7)**

---

### **1️⃣ Sort an Array in Ascending Order Using Bubble Sort**

* **Input:** `[5, 3, 8, 4, 2]`
* **Output:** `[2, 3, 4, 5, 8]`

---

### **2️⃣ Sort an Array in Descending Order Using Bubble Sort**

* **Input:** `[7, 1, 5, 3, 2]`
* **Output:** `[7, 5, 3, 2, 1]`

---

### **3️⃣ Sort a String Alphabetically Using Bubble Sort Logic**

* **Input:** `"javascript"`
* **Output:** `"aacijprstv"`
  *(Treat string as array of characters.)*

---

### **4️⃣ Sort an Array and Count the Number of Swaps Performed**

* **Input:** `[4, 3, 2, 1]`
* **Output:**

  * Sorted: `[1,2,3,4]`
  * Swaps: `6`

---

### **5️⃣ Sort Only the Even Numbers in an Array (Odd Numbers Stay in Place)**

* **Input:** `[9, 4, 2, 7, 6, 5]`
* **Output:** `[9, 2, 4, 7, 6, 5]`
  *(Sort evens using bubble sort while keeping odd numbers fixed.)*

---

### **6️⃣ Sort an Array of Strings by Length (Shortest to Longest)**

* **Input:** `["hi", "javascript", "is", "fun"]`
* **Output:** `["hi", "is", "fun", "javascript"]`

---

# 🏠 **Homework Questions (7)**

---

### **1️⃣ Sort an Array of Objects by Age (Ascending)**

* **Input:**

  ```js
  [
    {name: "A", age: 25},
    {name: "B", age: 20},
    {name: "C", age: 30}
  ]
  ```
* **Output:** Sorted by `age`.

---

### **2️⃣ Sort an Array but Keep Zeros at the End (Zeros Fixed)**

* **Input:** `[3, 0, 5, 0, 2, 1]`
* **Output:** `[1, 2, 3, 5, 0, 0]`

---

### **3️⃣ Sort an Array and Track How Many Comparisons Were Made**

* **Input:** `[5, 1, 4, 2, 8]`
* **Output:**

  * Sorted array
  * Total comparisons

---

### **4️⃣ Sort an Array of Characters Case-Insensitive**

* **Input:** `['b', 'A', 'd', 'C']`
* **Output:** `['A', 'b', 'C', 'd']`

---

### **5️⃣ Sort a 2D Array by the First Element of Each Subarray**

* **Input:**

  ```
  [[3, "c"], [1, "a"], [2, "b"]]
  ```
* **Output:**

  ```
  [[1, "a"], [2, "b"], [3, "c"]]
  ```

---

### **6️⃣ Sort an Array in Ascending Order but Stop After K Passes**

* **Input:** `[5, 4, 3, 2, 1]`, K = 2
* **Output:** Array state after 2 bubble sort passes.